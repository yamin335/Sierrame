...
import com.mmfinfotech.streameApp.util.validationEmptyField
                ...
                when (status) {
                    Sccess -> {
                        ...
                        for (i in 0 until dataArray?.size()!!) {
                            ...
                            val update_on
                                     = recordData?.get("update_on")?.asInt.toString()
...
////                            arrComments?.size?.minus(1)!!
////                        )
                        if (arrComments?.contains(null) == true) {
                            arrComments?.remove(null)
                        }
                        if (pageNo == 1)
                            adapterComments?.notifyDataSetChanged()
                        else {
